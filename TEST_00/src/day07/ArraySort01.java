...
/*
 	예제]
 		...
 		낮은 번호부터 정렬해서 담긴 데이터를 출력해보자
 		
 		로또 한 게임은
 			1 ~ 45 까지의 숫자가 랜덤 6개 필요
 */
import java.util.*;
import javax.swing.*;
public class ArraySort01 {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<lotto.length-1; i++) {
			...
			// 꺼낸 데이터 이후부터 맨 마지막까지 하나씩 꺼내서 비교
			for(int j=i+1; j<lotto.length; j++) {
				if(lotto[i]>lotto[j]) {
					// 이 경우는 뒤에서 뽑아온 데이터가 더 작은 경우이므로
					// 현재 자리와 데이터를 교체해야 한다
					// 이 때 현재 자리에 데이터를 교체하는 순간
					// 이전 데이터의 기억이 사라지므로 변수에 주소를 기억시켜놓는다
					int tmp = lotto[i];
					lotto[i] = lotto[j];
					lotto[j] = tmp;
				}
		...
		// 7. 출력
		JOptionPane.showMessageDialog(null,
				"<html><b style=\"color: red;\">**당첨번호**</b></html>\n" + Arrays.toString(lotto));
