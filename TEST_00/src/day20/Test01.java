...
public class Test01 {
	...
	public Test01() {
		...
		try {
			 ...
			 */
			JOptionPane.showMessageDialog(null, "Oracle Connection Complete");
			
			// 이제 오라클에 접속도 되었으니 질의명령을 보내보자
			// 3. Statement 만든다
			...
			// 이제 Statement에 질의명령을 실어서 보내면 된다
			/*
			int cnt = 0;
...
//			stmt.execute(sql03);
			...
			String sql04 = "SELECT mno, name, id, pw, mail email, gen, tel as HP, ano avatarno, isshow status FROM member WHERE mno = 1000";
			ResultSet rs = stmt.executeQuery(sql04);
			// 현재 작업줄은 BOF이므로 실제 데이터가 들어있는 행으로 한 행 내려야 된다
			rs.next();
